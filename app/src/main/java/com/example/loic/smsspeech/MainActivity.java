...
//        Au demarrage, je charge la vue principale avec ses éléments
...
//		En fonction du bouton sur lequel on a cliqu� on choisit un ensemble d'actions
		...
		case R.id.btnActiver:				// Si le user clique sur le bouton "activer" sans avoir saisi une phrase une erreur est signal�e 
			...
			if(strTexte.equals("")){
				...
				.setMessage("Ce champ ne doit pas etre vide!")
				.setPositiveButton("OK", null).show();
				edtPhrase.setEnabled(true);
				edtPhrase.requestFocus();
			}
			else {										// Sinon, je ne laisse au user que la possibilit� de d�sactiver le service
...
//    	A l'arret du service je romp la connexion entre l'activit� et le service
    	
    	unbindService(mConnexion);
    }
    
//    Initialisation de la variable de connexion entre l'activit� et le service
